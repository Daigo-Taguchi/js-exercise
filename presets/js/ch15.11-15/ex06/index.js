...
document.addEventListener("DOMContentLoaded", async () => {
  // sessionStorage からタスクの一覧を JSON オブジェクトで取得する
  ...
  Object.keys(sessionStorage).forEach((id) => {
    const task = JSON.parse(sessionStorage.getItem(id));

    // sessionStorage 内の value の構造が {name: "hoge", status: "active"} の形じゃなければ skip
    ...
    toggle.addEventListener("change", () => {
      ...
      // sessionStorage 内のタスクの状態を更新する
      sessionStorage.setItem(id, JSON.stringify({ name: task.name, status }));
    });
...
form.addEventListener("submit", (e) => {
  ...
  // 新しいタスクを sessionStorage に保存
  sessionStorage.setItem(id, JSON.stringify({ name: todo, status: "active" }));
  ...
  toggle.addEventListener("change", () => {
    ...
    // セッションストレージを更新
    sessionStorage.setItem(id, JSON.stringify({ name: todo, status }));
  });
